    ...
    <div>
      <nav className="bg-primary shadow-md sticky top-0 z-50">
        <div className="flex items-center justify-between h-20 px-12">
          {/* Logo / Brand - positioned on the far left */}
          <Link to='/' className="flex items-center gap-3 text-white font-bold text-2xl tracking-tight" onClick={closeMenu}>
            <img src="/klu-logo.jpg" alt="KLU Logo" className="h-12 object-contain" />
            Academic Tools
          </Link>
          ...
          <div className="hidden md:flex items-center gap-4">
            ...
            <Link to='/percentage' className={`px-4 py-2 rounded-full text-sm font-medium transition-colors duration-300 ${getDesktopClass('/percentage')}`}>CGPA to Percentage</Link>
          </div>
        ...
        {isOpen && (
          <div className="md:hidden bg-white border-t border-gray-100 shadow-lg">
            <div className="px-2 pt-2 pb-3 space-y-1 sm:px-3 flex flex-col">
              ...
              <Link to='/percentage' onClick={closeMenu} className={`block px-3 py-2 rounded-xl text-base font-medium transition-colors ${getMobileClass('/percentage')}`}>CGPA to Percentage</Link>
            </div>
          </div>
        )}
      </nav>
